...
| Repo | Poll Interval | Last Poll | Issues Seen | Last PR Seen | Project Path | Min Level |
|------|--------------|-----------|-------------|--------------|--------------|-----------|
| owner/repo | 5m | 2 min ago | 142 | 8 | ~/Projects/repo | 3 |

For each repo, also read `last_pr_seen` and `project_path` from the state file.

### Pending Approvals

Group pending approvals by type.

**Issues:**

Show items where `type == "issue"` or `type` is null/missing:
...
If none: "No pending issue approvals."

**PR Reviews:**

Show items where `type == "pr"`:

| # | PR | Lenses Flagged | Review Level | Summary |
|---|-----|---------------|-------------|---------|
| 1 | #51 "Refactor auth" | security, correctness | request-changes | Missing input validation on auth token |
| 2 | #53 "Add caching" | performance | comment | Consider cache eviction strategy |

If none: "No pending PR reviews."
