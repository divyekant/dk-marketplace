...
# ── load_policy cascade — project root first ───────────────────────
CASCADE_PROJECT="$TEMP_DIR/cascade-project"
mkdir -p "$CASCADE_PROJECT/.argos"
cat > "$CASCADE_PROJECT/.argos/policy.yaml" <<'YAML'
repo: cascade/test
floors:
  minimum: 3
YAML
POLICY_JSON=$(load_policy "" "$CASCADE_PROJECT")
MIN=$(echo "$POLICY_JSON" | get_minimum_floor)
if [[ "$MIN" != "3" ]]; then
  echo "FAIL: cascade project-root expected minimum=3, got $MIN"
  exit 1
fi
echo "PASS: load_policy cascade — project root .argos/policy.yaml wins (minimum=3)"

# ── load_policy cascade — fallback to global ────────────────────────
GLOBAL_DIR="$TEMP_DIR/global-config"
mkdir -p "$GLOBAL_DIR"
cat > "$GLOBAL_DIR/policy.yaml" <<'YAML'
repo: global/fallback
floors:
  minimum: 2
YAML
NO_ARGOS_PROJECT="$TEMP_DIR/no-argos-project"
mkdir -p "$NO_ARGOS_PROJECT"
POLICY_JSON=$(load_policy "$GLOBAL_DIR/policy.yaml" "$NO_ARGOS_PROJECT")
MIN=$(echo "$POLICY_JSON" | get_minimum_floor)
if [[ "$MIN" != "2" ]]; then
  echo "FAIL: cascade fallback expected minimum=2, got $MIN"
  exit 1
fi
echo "PASS: load_policy cascade — fallback to global policy (minimum=2)"

# ── load_policy cascade — in-repo wins over global ──────────────────
BOTH_PROJECT="$TEMP_DIR/both-project"
mkdir -p "$BOTH_PROJECT/.argos"
cat > "$BOTH_PROJECT/.argos/policy.yaml" <<'YAML'
repo: both/inrepo
floors:
  minimum: 4
YAML
GLOBAL_BOTH="$TEMP_DIR/global-both"
mkdir -p "$GLOBAL_BOTH"
cat > "$GLOBAL_BOTH/policy.yaml" <<'YAML'
repo: both/global
floors:
  minimum: 1
YAML
POLICY_JSON=$(load_policy "$GLOBAL_BOTH/policy.yaml" "$BOTH_PROJECT")
MIN=$(echo "$POLICY_JSON" | get_minimum_floor)
if [[ "$MIN" != "4" ]]; then
  echo "FAIL: cascade in-repo-wins expected minimum=4, got $MIN"
  exit 1
fi
echo "PASS: load_policy cascade — in-repo (minimum=4) wins over global (minimum=1)"

# ── PR policy accessors ─────────────────────────────────────────────
# Create a policy JSON with full prs section
PR_POLICY='{"prs":{"enabled":true,"ignore_authors":["dependabot","renovate"],"ignore_labels":["wip","draft"],"review":{"noise_budget":5,"auto_approve":true,"lenses":["security","correctness","style"]}}}'

VAL=$(echo "$PR_POLICY" | get_pr_enabled)
if [[ "$VAL" != "true" ]]; then
  echo "FAIL: get_pr_enabled expected true, got $VAL"
  exit 1
fi
echo "PASS: get_pr_enabled returns true"

VAL=$(echo "$PR_POLICY" | get_pr_ignore_authors)
if [[ "$VAL" != "dependabot,renovate" ]]; then
  echo "FAIL: get_pr_ignore_authors expected dependabot,renovate, got $VAL"
  exit 1
fi
echo "PASS: get_pr_ignore_authors returns dependabot,renovate"

VAL=$(echo "$PR_POLICY" | get_pr_ignore_labels)
if [[ "$VAL" != "wip,draft" ]]; then
  echo "FAIL: get_pr_ignore_labels expected wip,draft, got $VAL"
  exit 1
fi
echo "PASS: get_pr_ignore_labels returns wip,draft"

VAL=$(echo "$PR_POLICY" | get_pr_noise_budget)
if [[ "$VAL" != "5" ]]; then
  echo "FAIL: get_pr_noise_budget expected 5, got $VAL"
  exit 1
fi
echo "PASS: get_pr_noise_budget returns 5"

VAL=$(echo "$PR_POLICY" | get_pr_auto_approve)
if [[ "$VAL" != "true" ]]; then
  echo "FAIL: get_pr_auto_approve expected true, got $VAL"
  exit 1
fi
echo "PASS: get_pr_auto_approve returns true"

VAL=$(echo "$PR_POLICY" | get_pr_lenses)
if [[ "$VAL" != "security,correctness,style" ]]; then
  echo "FAIL: get_pr_lenses expected security,correctness,style, got $VAL"
  exit 1
fi
echo "PASS: get_pr_lenses returns security,correctness,style"

# Test defaults when prs section is missing
EMPTY_PR_POLICY='{}'

VAL=$(echo "$EMPTY_PR_POLICY" | get_pr_enabled)
if [[ "$VAL" != "false" ]]; then
  echo "FAIL: get_pr_enabled default expected false, got $VAL"
  exit 1
fi
echo "PASS: get_pr_enabled defaults to false"

VAL=$(echo "$EMPTY_PR_POLICY" | get_pr_ignore_authors)
if [[ "$VAL" != "" ]]; then
  echo "FAIL: get_pr_ignore_authors default expected empty, got $VAL"
  exit 1
fi
echo "PASS: get_pr_ignore_authors defaults to empty"

VAL=$(echo "$EMPTY_PR_POLICY" | get_pr_ignore_labels)
if [[ "$VAL" != "" ]]; then
  echo "FAIL: get_pr_ignore_labels default expected empty, got $VAL"
  exit 1
fi
echo "PASS: get_pr_ignore_labels defaults to empty"

VAL=$(echo "$EMPTY_PR_POLICY" | get_pr_noise_budget)
if [[ "$VAL" != "10" ]]; then
  echo "FAIL: get_pr_noise_budget default expected 10, got $VAL"
  exit 1
fi
echo "PASS: get_pr_noise_budget defaults to 10"

VAL=$(echo "$EMPTY_PR_POLICY" | get_pr_auto_approve)
if [[ "$VAL" != "false" ]]; then
  echo "FAIL: get_pr_auto_approve default expected false, got $VAL"
  exit 1
fi
echo "PASS: get_pr_auto_approve defaults to false"

VAL=$(echo "$EMPTY_PR_POLICY" | get_pr_lenses)
if [[ "$VAL" != "" ]]; then
  echo "FAIL: get_pr_lenses default expected empty, got $VAL"
  exit 1
fi
echo "PASS: get_pr_lenses defaults to empty"
